...
const NewPaste: FC<Props> = ({ background }) => {
  ...
  const onInsertPassword = (newPassword: string) => {
    setData((state) => {
      let newMessage = state.message;
      if (generatedPassword && newMessage.includes(generatedPassword)) {
        newMessage = newMessage.replace(generatedPassword, newPassword);
      } else {
        newMessage = newMessage ? `${newMessage}${newPassword}` : newPassword;
      }
  ...
  useEffect(() => {
    if (redirect) {
      navigate(`/${pasteId}#${key}`, {
        replace: false,
        state: { newPaste: true, destroy, period, background },
      });
    }
          ...
          <div id="paste_container">
            ...
            <div className="paste_container-main">
              <Paper id="new_paste_box">
                ...
                <PasswordGenerator onInsert={onInsertPassword} />
              ...
              <Paper id="new_paste_option">
                <div className="validity-section">
                  <div className="desc">
                    <p className="settings-title">
                      <strong>{t("paste.label.validity")}</strong>
                    </p>
                    <p className="settings-text">{t("paste.text.validity")}</p>
                  </div>
                </div>
                ...
                <div className="destroy-section">
                  <div className="desc">
                    <p className="settings-title">
                      <strong>{t("paste.label.destroy")}</strong>
                    </p>
                  </div>
                  <StyledSwitch onChange={onChangeDestroy} checked={destroy} />
                </div>
                <div className="password-section">
                  <div className="desc">
                    <p className="settings-title">
                      <strong>{t("paste.label.password")}</strong>
                    </p>
                  </div>
                  <StyledSwitch onChange={onTogglePassword} checked={enablePassword} />
                </div>
                {enablePassword && (
                  <>
                    <div className="password-section">
